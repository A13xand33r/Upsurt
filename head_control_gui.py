...
class App(tk.Tk):
    def __init__(self):
        ...
        self.after(150, self.poll_tracking_status)
    ...
    def show_main_menu(self):
        ...
        if self.status_card is not None and not self.status_card.winfo_ismapped():
            try:
                self.status_card.pack(fill="x", pady=(0, 20))
            except Exception:
                pass
        if self.bottom_frame is not None and not self.bottom_frame.winfo_ismapped():
            try:
    ...
    def open_keyboard_window(self):
        ...
        self.keyboard_view.set_keyboard_state(self.keyboard_is_open)
        ...
        self.set_mode("Keyboard menu", "#34d399")
        self.set_status("Клавиатура: готова")
    ...
    def update_light_status_label(self):
        ...
        self.set_status(f"Лампа: {state_text}", state_color)
        if self.light_view is not None:
            self.light_view.set_light_state(self.light_is_on)
    ...
    def build_ui(self):
        ...
        self.status_card = tk.Frame(main, bg="#121a2e", bd=0, highlightthickness=0)
        self.status_card.pack(fill="x", pady=(0, 20))

        left_status = tk.Frame(self.status_card, bg="#121a2e")
        ...
        right_status = tk.Frame(self.status_card, bg="#121a2e")
        ...
        self.bottom_frame = tk.Frame(main, bg="#0b1020")
        self.bottom_frame.pack(fill="x", pady=(18, 0))

        controls = tk.Frame(self.bottom_frame, bg="#0b1020")
        controls.pack(side="left")

        btn_toggle = self.make_small_button(controls, "Toggle Mouse", "#1f2937", self.on_toggle)
        btn_toggle.pack(side="left", padx=(0, 10))
        self.main_click_targets.append(btn_toggle)

        btn_calibrate = self.make_small_button(controls, "Calibrate", "#374151", self.on_calibrate)
        btn_calibrate.pack(side="left")
        self.main_click_targets.append(btn_calibrate)

        self.btn_exit = self.make_small_button(self.bottom_frame, "Exit", "#7f1d1d", self.on_exit)
        self.btn_exit.pack(side="right")
        self.main_click_targets.append(self.btn_exit)
        ...
        self.climate_view = ClimateMenuView(
            ...
            on_warm_air=self.on_climate_warm_air,
            on_cold_air=self.on_climate_cold_air,
        ...
        self.keyboard_view = KeyboardMenuView(
            main,
            on_keyboard_open=self.on_keyboard_open,
    ...
    def on_climate_temp_up(self):
        try:
            ...
            try:
                ...
                pass
            self.set_status("Климатик: Темп +", "#60a5fa")
        except Exception as e:
    ...
    def on_climate_power_on(self):
        try:
            ...
            try:
                ...
                pass
            self.set_status("Климатик: Включи", "#60a5fa")
        except Exception as e:
            self.set_status(f"Climate failed: {e}", "#ff6b6b")

    def on_climate_power_off(self):
        try:
            send_cmd("climate_off")
            self.climate_is_on = False
            try:
                if self.climate_view is not None:
                    self.climate_view.set_power_state(self.climate_is_on)
                ...
                pass
            self.set_status("Климатик: Изключи", "#60a5fa")
        except Exception as e:
            self.set_status(f"Climate failed: {e}", "#ff6b6b")
